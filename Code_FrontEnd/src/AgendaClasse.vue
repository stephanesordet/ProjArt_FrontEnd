...
const { data: cours } = useFetch('http://127.0.0.1:8000/api/cours/user/' + sessionStorage.getItem('user'))

const tabCours = window.addEventListener('hashchange', () => {
    if (window.location.hash == '#agendaClasse' && sessionStorage.getItem('user')) {
        session.value = true
    } else {
        session.value = false
    }
